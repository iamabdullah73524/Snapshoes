...
import { Star, ShoppingCart, Plus, Minus, ShieldCheck, Heart, AlertTriangle } from 'lucide-react';
    ...
    return (
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-20 flex justify-center items-center">
        <div className="animate-pulse flex flex-col items-center space-y-4">
          ...
          <span className="text-xs font-bold uppercase tracking-widest text-gray-400">Loading Shoe details...</span>
        </div>
      </div>
    );
  }
  ...
  if (!product) {
    return (
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-20 text-center space-y-4">
        ...
        <h2 className="text-xl font-bold uppercase tracking-wider text-brand-dark">Shoe Not Found</h2>
        <p className="text-xs text-gray-400 font-medium">The shoe you are searching for might be discontinued or deleted.</p>
        <Link to="/shop" className="inline-block bg-brand-dark text-white text-[10px] font-extrabold uppercase tracking-widest px-6 py-3 rounded-lg hover:bg-brand-orange transition">
          Return to Shop
        </Link>
      </div>
    );
    ...
    <div className="max-w-7xl mx-auto px-6 md:px-12 py-10 space-y-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            ...
            <div className="flex items-center space-x-2 pt-1">
              ...
              <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">
                {product.rating ? `${product.rating} Rating` : 'No rating yet'} ({reviews.length} reviews)
              </span>
            </div>
          </div>
          ...
          <div className="p-4 bg-brand-gray border border-gray-150 rounded-2xl flex items-center justify-between">
            <div>
              <p className="text-[9px] font-bold text-gray-400 uppercase tracking-wider">Retail Price</p>
              <div className="flex items-baseline space-x-2.5">
                {hasSale ? (
                  <>
                    <span className="text-2xl font-black text-red-500">₹{product.salePrice.toFixed(2)}</span>
                    <span className="text-sm text-gray-400 line-through font-semibold">₹{product.price.toFixed(2)}</span>
                  </>
                ) : (
            ...
            <div className="text-right">
              <span className={`text-[10px] font-extrabold uppercase px-3 py-1 rounded-full ${product.inventory > 0 ? 'bg-emerald-50 text-emerald-600 border border-emerald-150' : 'bg-red-50 text-red-600 border border-red-150'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 pt-8 border-t border-gray-200">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div>
            <h2 className="text-lg font-black uppercase tracking-wider text-brand-dark">Customer Feedback</h2>
            <p className="text-xs text-gray-400 mt-1 font-medium leading-relaxed">
              We value your opinions. Let us know how this performance model fits and behaves during action.
            </p>
          </div>

          {user ? (
            <form onSubmit={handleReviewSubmit} className="bg-white border border-gray-150 p-6 rounded-2xl shadow-sm space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Your Rating</label>
                <div className="flex items-center space-x-1.5">
                  {[1, 2, 3, 4, 5].map((num) => (
                    <button
                      ...
                      className={`p-0.5 text-2xl transition-all ${rating >= num ? 'text-amber-400 scale-105' : 'text-gray-200'}`}
                    >
                      ★
                    </button>
                  ))}
              ...
              {/* Comment Input */}
              <div className="space-y-1.5">
                <label className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Comments</label>
                <textarea
              ...
              <button
                ...
                className="w-full bg-brand-dark text-white py-3 rounded-lg text-[10px] font-extrabold uppercase tracking-widest hover:bg-brand-orange transition"
              >
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 text-center space-y-3.5">
              <p className="text-xs font-semibold text-amber-700 leading-relaxed">
                You must be logged in to share ratings and reviews for this shoe model.
              </p>
              <Link 
                to="/login" 
          ...
          <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
            {reviews.length === 0 ? (
              <div className="text-center py-12 bg-white border border-gray-150 rounded-2xl">
                ...
                <p className="text-[10px] text-gray-400 mt-1 font-medium">Be the first to submit a review for this shoe!</p>
              </div>
            ) : (
              reviews.map((rev) => (
                <div key={rev._id} className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="text-xs font-bold text-brand-dark">{rev.userName}</h4>
